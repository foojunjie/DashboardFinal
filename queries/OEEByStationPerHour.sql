WITH downtime_periods AS (
  ...
  WHERE EXTRACT(HOUR FROM anomaly_timestamp) = %s 
  and anomaly_timestamp >= %s::date 
...
first_output_cycle AS (
  ...
  WHERE EXTRACT(HOUR FROM start_ts) = %s 
  and start_ts >= %s::date 
...
last_output_cycle AS (
  ...
  FROM jtc_output_cycle
  WHERE EXTRACT(HOUR FROM end_ts) = %s 
  and end_ts >= %s::date 
...
first_leak_test AS (
  ...
  FROM leak_test_log
  WHERE EXTRACT(HOUR FROM modified_at) = %s 
  and modified_at >= %s::date 
...
station_metrics AS (
  ...
  and EXTRACT(HOUR FROM atc_timestamp_start) = %s or EXTRACT(HOUR FROM atc_timestamp_end) = %s
