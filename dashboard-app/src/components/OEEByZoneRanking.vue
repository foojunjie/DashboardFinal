  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        const OEEData = {
          name: sortedOEE.map(i => `${i.workcell} Zone ${i.zone}`),
          value: sortedOEE.map(i => i.oee)
        };
        const AVData = {
          name: sortedAvailability.map(i => `${i.workcell} Zone ${i.zone}`),
          value: sortedAvailability.map(i => i.availability)
        };
        const PFData = {
          name: sortedPerformance.map(i => `${i.workcell} Zone ${i.zone}`),
          value: sortedPerformance.map(i => i.performance)
        };
        const QLData = {
          name: sortedQuality.map(i => `${i.workcell} Zone ${i.zone}`),
