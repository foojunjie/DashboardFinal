...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
      const response = await fetch('http://127.0.0.1:8000/api/OEE_by_WorkCell')
      ...
      const OEEData = {
        name: sortedOEE.map(i => i.workcell),
        value: sortedOEE.map(i => i.oee)
      };
      const AVData = {
        name: sortedAvailability.map(i => i.workcell),
        value: sortedAvailability.map(i => i.availability)
      };
      const PFData = {
        name: sortedPerformance.map(i => i.workcell),
        value: sortedPerformance.map(i => i.performance)
      };
      const QLData = {
        name: sortedQuality.map(i => i.workcell),
    ...
    initBarCharts() {
      ...
      charts.forEach(({ ref, data, title }) => {
        ...
        const paired = data.value.map((val, i) => ({
          val,
          label: data.name[i] // use workcell names from API
        }));
